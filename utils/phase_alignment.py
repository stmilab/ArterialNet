...
# based on https://github.com/pearsonkyle/Signal-Alignment
def equalize_array_size(array1, array2):
    ...
    if dif_length < 0:
        array2 = array2[
            int(np.floor(-dif_length / 2)) : len2 - int(np.ceil(-dif_length / 2))
        ]
    elif dif_length > 0:
        array1 = array1[
            int(np.floor(dif_length / 2)) : len1 - int(np.ceil(dif_length / 2))
        ]
...
def phase_align(reference, target, roi, res=100):
    ...
    return np.argmax(cc) * mod * (1.0 / res)
...
if __name__ == "__main__":
    ...
    shifted = shift(signal.gaussian(NPTS, std=4), SHIFTVAL) + np.random.normal(
        1, NOISE, NPTS
    )
...
def region_of_interest(wave1, wave2, outlier_thre):
    sliding_list = []
    for i, e in enumerate(wave2):
        if i == 0:
...
def find_phase_align(wave1, wave2, outlier_thre=None, roi=None, res=1000):
    if outlier_thre == None:
        outlier_thre = max(1, wave2.mean() + 1.96 * wave2.std())
        print(f"outlier_thre={outlier_thre}")
    if roi == None:
        sliding_list = region_of_interest(
            wave1=wave1, wave2=wave2, outlier_thre=outlier_thre
        )
    st = sliding_list[0]
    ed = sliding_list[-1]
    # print(sliding_list)
    if len(sliding_list) == 2:
        s = phase_align(
            wave1[st:ed], wave2[st:ed], roi=[st + ed / 4, ed * 3 / 4], res=1000
        )
    else:
        for i, e in enumerate(sliding_list[:-1]):
            if sliding_list[i + 1] - sliding_list[i] > 30:
                s = phase_align(
                    wave1[st:ed],
                    wave2[st:ed],
                    roi=[sliding_list[i], sliding_list[i + 1]],
                    res=1000,
                )
                break
    return abs(s)
